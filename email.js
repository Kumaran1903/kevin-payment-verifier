...
export async function sendPaymentEmail({
  ...
  const response = await resend.emails.send({
    ...
    subject: `🧾 Payment Submission from ${name}`,
    html: `
      ...
      <a href="${process.env.BASE_URL}/api/decision?status=accept&email=${encodeURIComponent(
        email
      )}&key=${process.env.VERIFICATION_SECRET}" style="padding:10px 20px;background:green;color:white;text-decoration:none;border-radius:6px;">✅ Accept</a>
      &nbsp;
      <a href="${process.env.BASE_URL}/api/decision?status=reject&email=${encodeURIComponent(
        email
      )}&key=${process.env.VERIFICATION_SECRET}" style="padding:10px 20px;background:red;color:white;text-decoration:none;border-radius:6px;">❌ Reject</a>
    `,
    attachments: [
      {
